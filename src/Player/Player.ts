...
export interface StartSpeakingEventVoiceReceiver
  extends BaseVoiceReceiverEvent {
...
export type VoiceReceiverEvent =
...
export class Player extends EventEmitter {
  ...
  /** The timestamp of last time played song */
  ...
  public async play(): Promise<Player> {
    ...
    if (
      ...
      this.currentTrack?.info.sourceName === "youtube"
    ) {
      const data = await this.poru.client.localTrackManager.resolve(
        this.currentTrack
      );
      if (data) {
  ...
  ): Promise<NodeLinkGetLyrics | null> {
    let node: Node | undefined = this.node;

    if (!this.node.isNodeLink)
      node = (Array.from(this.poru.nodes) as [string, Node][])?.find(
        ([, node]) => node.isNodeLink
      )?.[1];
    if (!node || !node.isNodeLink)
      throw new Error(
        "[Poru Exception] No NodeLink node found in the Poru instance."
      );

    if (!encodedTrack && !this.currentTrack)
      throw new Error(
        "[Poru Exception] A track must be playing right now or be supplied."
      );

    encodedTrack = this.currentTrack?.track;

    return await this.node.rest.get<NodeLinkGetLyrics>(
      `/v4/lyrics?track=${encodeURIComponent(encodedTrack ?? "")}`
    );
  }
  ...
  public setVoiceChannel(
    ...
    options?: Required<
      Omit<
        ConnectionOptions,
        "guildId" | "region" | "textChannel" | "voiceChannel"
      >
    >
  ): this {
  ...
  public async autoplay(): Promise<Player> {
    try {
      ...
      const response = await this.poru.resolve({
        query: data,
        requester:
          this.previousTrack?.info?.requester ??
          this.currentTrack?.info?.requester,
        source:
          this.previousTrack?.info?.sourceName ??
          this.currentTrack?.info?.sourceName ??
          this.poru.options?.defaultPlatform ??
          "ytmsearch",
      });
  ...
  public async resolve({
    ...
    source,
    requester,
  }: ResolveOptions): Promise<Response> {
    const response = (await this.node.rest.get<LoadTrackResponse>(
      `/v4/loadtracks?identifier=${encodeURIComponent(
        (this.startsWithMultiple(query, ["https://", "http://"])
          ? ""
  ...
  public send(data: any): void {
    if (!this.poru.send)
      throw new Error(
        "[Poru Error] The send function is required to send data to discord. Please provide a send function in the Poru options or use one of the supported Libraries."
      );
  ...
  public async setupVoiceReceiverConnection() {
    return new Promise<boolean>(async (resolve, reject) => {
      if (!this.node.isNodeLink)
        return reject(
          new Error(
            "[Poru Exception] This function is only available for NodeLink nodes."
          )
        );
      if (!this.poru.userId)
        return reject(
          new Error(
            "[Poru Error] No user id found in the Poru instance. Consider using a supported library."
          )
        );
      ...
      this.voiceReceiverWsClient = new WebSocket(
        `${secure ? "wss" : "ws"}://${host}:${port}/connection/data`,
        { headers }
      );
      this.voiceReceiverWsClient.on("open", this.voiceReceiverOpen.bind(this));
      this.voiceReceiverWsClient.on(
        "error",
        this.voiceReceiverError.bind(this)
      );
      this.voiceReceiverWsClient.on(
        "message",
        this.voiceReceiverMessage.bind(this)
      );
      this.voiceReceiverWsClient.on(
        "close",
        this.voiceReceiverClose.bind(this)
      );
  ...
  private async voiceReceiverClose(event: any): Promise<void> {
    try {
      ...
      this.poru.emit(
        "debug",
        "[Voice Receiver Web Socket] Error while closing the connection with the node.",
        error
      );
    }
  }
  ...
  private async voiceReceiverReconnect(): Promise<void> {
    this.voiceReceiverReconnectTimeout = setTimeout(async () => {
      ...
      this.poru.emit(
        "debug",
        this.node.name,
        `[Voice Receiver Web Socket] Reconnecting to the voice Receiver Websocket...`
      );
  ...
  private async voiceReceiverDisconnect(): Promise<void> {
    ...
    this.voiceReceiverWsClient = null;
    this.poru.emit(
      "voiceReceiverDisconnected",
      this,
      `[Voice Receiver Web Socket] Connection was closed.`
    );
  }
  ...
  private async voiceReceiverOpen(): Promise<void> {
    try {
      ...
      this.poru.emit(
        "voiceReceiverConnected",
        this,
        `[Voice Receiver Web Socket] Connection ready ${this.node.socketURL}/connection/data`
      );
    } catch (error) {
      this.poru.emit(
        "debug",
        `[Voice Receiver Web Socket] Error while opening the connection with the node ${this.node.name}. to the voice Receiver Websocket.`,
        error
      );
    }
  }
  ...
  private async voiceReceiverMessage(payload: string): Promise<void> {
    try {
      ...
      this.poru.emit(
        "debug",
        this.node.name,
        `[Voice Receiver Web Socket] Recieved a payload: ${payload}`
      );
      ...
      switch (packet.type) {
        ...
        default: {
          this.poru.emit(
            "debug",
            this.node.name,
            `[Voice Receiver Web Socket] Recieved an unknown payload: ${payload}`
          );
          break;
        }
      }
      ...
      this.poru.emit(
        "voiceReceiverError",
        this,
        "[Voice Receiver Web Socket] Error while parsing the payload. " + err
      );
    }
  }
  /**
